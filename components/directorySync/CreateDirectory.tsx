...
const CreateDirectory: React.FC<CreateDirectoryProps> = ({
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      const res = await fetch(`/api/teams/${team.slug}/directory-sync`, {
        method: 'POST',
      ...
      <DialogContent className="sm:max-w-md">
        <form onSubmit={formik.handleSubmit}>
          <DialogHeader className="pt-4">
            <DialogTitle>{t('create-directory-connection')}</DialogTitle>
            <DialogDescription>
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="grid gap-1">
              <Label htmlFor="provider">{t('directory-sync-provider')}</Label>
              <Select
                name="provider"
                value={formik.values.provider}
                onValueChange={(val) => formik.setFieldValue('provider', val)}
              >
