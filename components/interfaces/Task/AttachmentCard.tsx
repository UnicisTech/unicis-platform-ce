...
const AttachmentsCard = ({
    ...
    const downloadHanlder = useCallback(async (event: MouseEvent<HTMLButtonElement>) => {
        try {
            ...
            const response = await axios.get(`/api/teams/${teamSlug}/tasks/${taskNumber}/attachments?id=${attachment.id}`, {
                responseType: 'blob',
            });
    ...
    const openDeleteModal = useCallback(async (event: MouseEvent<HTMLButtonElement>) => {
        event.preventDefault();
        event.stopPropagation();
