...
import { statusOptions, perPageOptions } from '@/components/defaultLanding/data/configs/csc';
import { ISO } from 'types';
import useCscStatuses from 'hooks/useCscStatuses';
import { Task } from '@prisma/client';
import CscChartsLayout from './CscChartsLayout';

export async function updateCscStatus(params: { slug: string; control: string; value: string; framework: ISO }) {
    const res = await fetch(`/api/teams/${params.slug}/csc`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ control: params.control, value: params.value, framework: params.framework }),
    });
    const json = await res.json();
    return { data: json.data, error: json.error || (!res.ok ? { message: 'Request failed' } : null) };
}

export async function updateTaskCsc(params: {
    slug: string; taskNumber: string | number; controls: string[]; operation: 'add' | 'remove'; iso: string
}) {
    const res = await fetch(`/api/teams/${params.slug}/tasks/${params.taskNumber}/csc`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ controls: params.controls, operation: params.operation, ISO: params.iso }),
    });
    const json = await res.json();
    return { data: json.data, error: json.error || (!res.ok ? { message: 'Request failed' } : null) };
}
...
export default function CscPanel({ slug, iso, tasks, mutateTasks }: CscPanelProps) {
    ...
    const taskSelectorHandler = useCallback(
        async (action: string, dataToChange: Array<{ value: number }>, control: string) => {
