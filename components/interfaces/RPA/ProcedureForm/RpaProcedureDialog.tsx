...
import {
  useDescriptionAndStakeholdersStepForm,
  usePurposeAndCategoriesStepForm,
  useRecipientsStepForm,
  useSecurityMeasuresStepForm,
  useTiaStepForm,
} from './hooks';
import {
...
export default function RpaProcedureDialog({
  ...
  const [procedureData, setProcedureData] = React.useState<any>(
    prevProcedure || defaultProcedure
  );
  const [task, setTask] = React.useState<Task | null>(selectedTask || null);
  const [isSaving, setIsSaving] = React.useState<boolean>(false);

  const taskForm = useForm<{ task: Task }>({ mode: 'onChange' });
  const descriptionAndStakeholdersForm =
    useDescriptionAndStakeholdersStepForm(procedureData);
  const purposeAndCategoriesForm =
    usePurposeAndCategoriesStepForm(procedureData);
