...
export default function useTransferScenarioStepForm(
  ...
  return useForm<TransferScenarioStepValues>({
    defaultValues: {
      ...
      CountryDataExporter: procedure[0]?.CountryDataExporter,
      DataImporter: procedure[0]?.DataImporter ?? '',
      CountryDataImporter: procedure[0]?.CountryDataImporter,
      TransferScenario: procedure[0]?.TransferScenario ?? '',
      DataAtIssue: procedure[0]?.DataAtIssue ?? '',
      HowDataTransfer: procedure[0]?.HowDataTransfer ?? '',
      StartDateAssessment:
        procedure[0]?.StartDateAssessment ??
        new Date().toISOString().slice(0, 10),
      AssessmentYears: procedure[0]?.AssessmentYears ?? 1,
      LawImporterCountry: procedure[0]?.LawImporterCountry,
    },
    mode: 'onSubmit',
  });
}
