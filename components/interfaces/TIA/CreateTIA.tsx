...
const CreateTIA = ({
  ...
  useEffect(() => {
    if (procedure[1]) {
      ...
      if (LawfulAccess === "no" || MassSurveillanceTelecommunications === "no" || SelfReportingObligations === "no") {
  ...
  useEffect(() => {
    if (procedure[2]) {
      ...
      const targetedRisk = Number(WarrantsSubpoenas) + Number(ViolationLocalLaw) + Number(HighViolationLocalLaw) + Number(HighViolationDataIssue) + Number(InvestigatingImporter) + Number(PastWarrantSubpoena)
      const nonTargetedRisk = Number(LocalIssueWarrants) + Number(LocalMassSurveillance) + Number(LocalAccessMassSurveillance) + Number(LocalRoutinelyMonitor) + Number(PassMassSurveillance)
      const selfReportingRisk = Number(ImporterObligation) + Number(LocalSelfReporting) + Number(PastSelfReporting)
  ...
  useEffect(() => {
    if (procedure[1]) {
      ...
      if (MassSurveillanceTelecommunications === "no" && ConnectionSurveillanceTele === "yes") {
        return setIsPermitted(false)
      }

      if (SelfReportingObligations === "no" && ConnectionSelfreportingObligations === "yes") {
        return setIsPermitted(false)
      }
              ...
              <Modal.Body>
                ...
                >
                  ...
                  {stage === 0 &&
                    <>
                      <Message
                        text={`To be completed by the exporter`}
                      />
                      {validationMessage &&
                        <Message
                          appearance="error"
                      ...
                      >
                        {({ fieldProps: { id, ...rest }, error }) => (
                          <Fragment>
                            <WithoutRing>
                              <Select inputId={id} {...rest} options={config.countries} validationState={error ? 'error' : 'default'} />
                              {error
                      ...
                      <Field
                        ...
                        defaultValue={procedure[0]?.StartDateAssessment || format(new Date(), 'yyyy-MM-dd')}
                        isRequired
                        aria-required={true}
                        validate={async (value) => {
                          if (value) {
                        ...
                        {({ fieldProps }) => {
                          ...
                          return (
                            <>
                              ...
                              />
                              <HelperMessage>
                                {`${fieldProps?.value} year${fieldProps?.value > 1 ? 's' : ''} (max. 10)`}
                              </HelperMessage>
                            </>
                          )
                        }}
                        ...
                        {({ fieldProps: { id, ...rest }, error }) => (
                          <Fragment>
                            <WithoutRing>
                              ...
                              {error
                                ? <ErrorMessage>{error}</ErrorMessage>
                                : <HelperMessage>Legal analysis on the lawful access laws of importer's country</HelperMessage>
                              }
                            </WithoutRing>
                          </Fragment>
                        )}
                    ...
                    <>
                      ...
                      >
                        {({ fieldProps }: { fieldProps: FieldProps<string> }) => (
                          <>
                            <HelperMessage>
                              Is the personal data at issue protected with adequate encryption in-transit (i.e. when transmitted)?v
                            </HelperMessage>
                            <RadioGroup
                              {...fieldProps}
                      ...
                      >
                        {({ fieldProps }: { fieldProps: FieldProps<string> }) => (
                          <>
                            <HelperMessage>
                              Is the personal data at issue protected by a transfer mechanism approved by applicable data protection law (e.g., the EU Standard Contractual Clauses in case of the GDPR, approved BCR, or - in the case of an onward transfer - a back-to-back-contract in line with the EU SCC), and compliance with it and its judicial enforcement be expected, insofar permitted under the importer's law?
                            </HelperMessage>
                            <RadioGroup
                              {...fieldProps}
                      ...
                      <p className="font-bold">
                        Based on the legal analysis above, the lawful access laws in the country of the importer are compatible with EU and CH law
                      </p>
                      ...
                      >
                        {({ fieldProps }: { fieldProps: FieldProps<string> }) => (
                          <>
                            <HelperMessage>
                              … in connection with targeted lawful access (e.g., investigations by the police, state prosecutors and other authorities)
                            </HelperMessage>
                            <RadioGroup
                              {...fieldProps}
                      ...
                      >
                        {({ fieldProps }: { fieldProps: FieldProps<string> }) => (
                          <>
                            <HelperMessage>
                              … in connection with mass surveillance of telecommunications, online services, etc. (e.g., by intelligence agencies)
                            </HelperMessage>
                            <RadioGroup
                              {...fieldProps}
                  ...
                  {stage === 2 &&
                    <>
                      <Message
                        text={`To be completed by the importer`}
                      />
                        ...
                        ? <>
                          ...
                          >
                            {({ fieldProps }: { fieldProps: FieldProps<string> }) => (
                              <>
                                <HelperMessage>
                                  The local authorities in principle have the right to issue to the importer or its providers warrants or subpoenas as described above for the type of data at issue
                                </HelperMessage>
                                <RadioGroup
                                  {...fieldProps}
                          ...
                          >
                            {({ fieldProps }: { fieldProps: FieldProps<string> }) => (
                              <>
                                <HelperMessage>
                                  The local authorities issue such requests regardless of whether they really believe that a violation of local law has occured or that there is threat for national security (e.g., for political reasons or as a scheme of extortion)
                                </HelperMessage>
                                <RadioGroup
                                  {...fieldProps}
                          ...
                          >
                            {({ fieldProps }: { fieldProps: FieldProps<string> }) => (
                              <>
                                <HelperMessage>
                                  The data subjects (of the data at issue) have a high probability of violating local laws and the data at issue would be particularly interesting to investigate these violations
                                </HelperMessage>
                                <RadioGroup
                                  {...fieldProps}
                          ...
                          >
                            {({ fieldProps }: { fieldProps: FieldProps<string> }) => (
                              <>
                                <HelperMessage>
                                  The importer has a high probability of violating local laws, and the data at issue would be particularly interesting to investigate these violations
                                </HelperMessage>
                                <RadioGroup
                                  {...fieldProps}
                          ...
                          >
                            {({ fieldProps }: { fieldProps: FieldProps<string> }) => (
                              <>
                                <HelperMessage>
                                  There are other reasons why the local authorities would be interested in investigating the importer (e.g., because it is considered of relevance for national security or economic espionage by the local government) and, therefore, requesting the data at issue from the importer or its providers
                                </HelperMessage>
                                <RadioGroup
                                  {...fieldProps}
                          ...
                          >
                            {({ fieldProps }: { fieldProps: FieldProps<string> }) => (
                              <>
                                <HelperMessage>
                                  In the past 5-10 years, the importer was already required to produce the type of data at issue following such a warrant or subpoena from a local authority (or, to the importer's best knowledge, one of its providers was required to grant access to the importer's data)
                                </HelperMessage>
                                <RadioGroup
                                  {...fieldProps}
                          ...
                          >
                            {({ fieldProps }: { fieldProps: FieldProps<string> }) => (
                              <>
                                <HelperMessage>
                                  Based on the above and the legal analysis, does the importer have reason to believe that during the assessment period it (or one of its providers) will have to produce some of the data at issue for an investigation as described above?
                                </HelperMessage>
                                <RadioGroup
                                  {...fieldProps}
                      ...
                      {procedure[1].MassSurveillanceTelecommunications === "no"
                        ? <>
                          <Message
                            isBold={true}
                            text={"Non-targeted lawful access (mass surveillance):</Strong> Risk of the importer receiving a request of an intelligence agency or other authority to participate in the routine monitoring of communications or other data)"}
                          />
                          ...
                          >
                            {({ fieldProps }: { fieldProps: FieldProps<string> }) => (
                              <>
                                <HelperMessage>
                                  The local authorities in principle have the right to issue to the importer warrants or subpoenas as described above for the type of data at issue
                                </HelperMessage>
                                <RadioGroup
                                  {...fieldProps}
                          ...
                          >
                            {({ fieldProps }: { fieldProps: FieldProps<string> }) => (
                              <>
                                <HelperMessage>
                                  The type of the data at issue is in principle of interest for mass surveillance (e.g., because it contains large volumes of third party communications or third party communications that could be of relevance for national security purposes)
                                </HelperMessage>
                                <RadioGroup
                                  {...fieldProps}
                          ...
                          >
                            {({ fieldProps }: { fieldProps: FieldProps<string> }) => (
                              <>
                                <HelperMessage>
                                  The local authorities could consider the importer to be a provider that has access to such type of data (e.g., because it is offering a corresponding service or is contracted to process such data)
                                </HelperMessage>
                                <RadioGroup
                                  {...fieldProps}
                          ...
                          >
                            {({ fieldProps }: { fieldProps: FieldProps<string> }) => (
                              <>
                                <HelperMessage>
                                  There are other reasons why the local authorities would be interested requiring the importer to routinely monitor the data at issue for and on behalf of the government (e.g., because it is considered of relevance for national security or economic espionage by the local government)
                                </HelperMessage>
                                <RadioGroup
                                  {...fieldProps}
                          ...
                          >
                            {({ fieldProps }: { fieldProps: FieldProps<string> }) => (
                              <>
                                <HelperMessage>
                                  In the past 5-10 years the importer was already required to engage in mass surveillance for the local authorities
                                </HelperMessage>
                                <RadioGroup
                                  {...fieldProps}
                          ...
                          >
                            {({ fieldProps }: { fieldProps: FieldProps<string> }) => (
                              <>
                                <HelperMessage>
                                  Based on the above and the legal analysis, does the importer have reason to believe that during the assessment period it will have to produce some of the data at issue in connection with such mass surveillance?
                                </HelperMessage>
                                <RadioGroup
                                  {...fieldProps}
                        ...
                        ? <>
                          ...
                          >
                            {({ fieldProps }: { fieldProps: FieldProps<string> }) => (
                              <>
                                <HelperMessage>
                                  The importer could in principle be subject to such a reporting obligation with regard to the type of data at issue
                                </HelperMessage>
                                <RadioGroup
                                  {...fieldProps}
                          ...
                          >
                            {({ fieldProps }: { fieldProps: FieldProps<string> }) => (
                              <>
                                <HelperMessage>
                                  The data at issue typically contains information that is subject to local self-reporting obligations (for the purpose of permitting the local authorities to investigate a matter; self-reporting obligations as part of prudential supervision are not in-scope)
                                </HelperMessage>
                                <RadioGroup
                                  {...fieldProps}
                          ...
                          >
                            {({ fieldProps }: { fieldProps: FieldProps<string> }) => (
                              <>
                                <HelperMessage>
                                  In the past 5-10 years the importer already self-reported data of this type to the authorities
                                </HelperMessage>
                                <RadioGroup
                                  {...fieldProps}
                          ...
                          >
                            {({ fieldProps }: { fieldProps: FieldProps<string> }) => (
                              <>
                                <HelperMessage>
                                  Based on the above and the legal analysis, does the importer have reason to believe that it will during the assessment period have to produce the data at issue as described above?
                                </HelperMessage>
                                <RadioGroup
                                  {...fieldProps}
                  ...
                  {stage === 3 &&
                    <>
                      ...
                      <p>
                        <span>
                          ... a problematic self-reporting obligation (according to the Importer):
                          {procedure[2].AssessmentProduceReport === "4"
                            ? <Badge appearance="added">Yes</Badge>
                            : <Badge appearance="removed">No</Badge>
                          }
                        ...
                        ? <>
                          ...
                          >
                            {({ fieldProps }: { fieldProps: FieldProps<string> }) => (
                              <>
                                <HelperMessage>
                                  (e.g., investigations by the police, state prosecutors and other authorities)
                                </HelperMessage>
                                <RadioGroup
                                  {...fieldProps}
                      ...
                      {procedure[1].SelfReportingObligations === 'no'
                        ? <>
                          <Field
                            ...
                            defaultValue={procedure[3]?.ConnectionSelfreportingObligations}
                          >
                            {({ fieldProps }: { fieldProps: FieldProps<string> }) => (
                              <>
                                <HelperMessage>
                          ...
                          <Field
                            ...
                            label={fieldPropsMapping["ReasonConnectionSelfreportingObligations"]}
                            defaultValue={procedure[3]?.ReasonConnectionSelfreportingObligations}
                          >
                            {({ fieldProps }) => (
                              <Fragment>
                                <TextArea
                    ...
                    <>
                      ...
                      <p>
                        <span className="font-bold">
                          In view of the above and applicable data protection laws, the transfer is:
                        </span>
                        {isPermitted
