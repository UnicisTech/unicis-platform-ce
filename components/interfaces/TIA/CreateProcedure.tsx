...
import Form, { Field, HelperMessage, ErrorMessage, FieldProps, RangeField, useFormState } from '@atlaskit/form';
...
import { shouldSkipTwoSteps, isTranferPermitted, getTargetedRisk, getNonTargetedRisk, getSelfReportingRisk, getTransferIsValue } from '@/lib/tia';
				...
				{({ fieldProps }) => {
					return (
						<>
							...
							<HelperMessage>
								{`${fieldProps?.value} year${fieldProps?.value > 1 ? 's' : ''
									} (max. 10)`}
							</HelperMessage>
						</>
					);
		...
		<>
			...
			{procedure[1]?.LawfulAccess === 'no' ? (
				<>
					...
					<RiskLevel value={getTargetedRisk()} />
					...
					{useFormState({ values: true })?.values?.DataIssueInvestigation === '2' && (
						<p>
							<Badge appearance="removed">Problematic Lawful Access expected!</Badge>
						</p>
					)}
				</>
			) : null}
			{procedure[1].MassSurveillanceTelecommunications === 'no' ? (
				<>
					...
					<RiskLevel value={getNonTargetedRisk()} />
					...
					{useFormState({ values: true })?.values?.PassMassSurveillanceConnection === '4' && (
						<p>
							<Badge appearance="removed">Problematic Lawful Access expected!</Badge>
						</p>
					)}
			...
			{procedure[1].SelfReportingObligations === 'no' ? (
				<>
					...
					<RiskLevel value={getSelfReportingRisk()} />
					...
					{useFormState({ values: true })?.values?.AssessmentProduceReport === '4' && (
						<p>
							<Badge appearance="removed">Problematic Lawful Access expected!</Badge>
						</p>
					)}
