...
export default function CreateCourse2({
    ...
    useEffect(() => {
        ...
        const newValues: QuestionStageValues = {
            ...
            // For TEXT:
            answer: currentQuestion.type === QuestionType.TEXT
            ...
            ...Object.fromEntries(
                (currentQuestion.type === QuestionType.SINGLE_CHOICE ||
                    ...
                    ? (currentQuestion as SingleChoiceQuestion | MultipleChoiceQuestion).answers.map(
                        (ans, i) => [
                            [`answer${i}`, ans.answer],
        ...
        if (currentQuestion.type === QuestionType.TEXT) {
            ...
            setAnswersAmount(
                (currentQuestion as SingleChoiceQuestion | MultipleChoiceQuestion | OrderQuestion).answers.length
            );
        }
    ...
    const createCourseStateWithQuestion = (courseState: any, questionObj: any) => {
    ...
    const saveCourseHandler = async (data: QuestionStageValues) => {
        ...
        const courseToSave = createCourseStateWithQuestion(courseState, createQuestion(data, answersAmount));
                ...
                {stage === 0 && (
                    ...
                    >
                        ...
                        <FormField
                            control={courseForm.control}
                            name="teams"
                            rules={selectedCourse ? undefined : { required: t("teams") + " is required." }}
                            render={({ field, fieldState }) => (
                                <FormItem>
                        ...
                        <FormField
                            ...
                            rules={{
                                validate: (v) =>
                                    !v || validateNumberField(String(v)) || t("Only numbers allowed"),
                            }}
                            render={({ field, fieldState }) => (
                                <FormItem>
                        ...
                        <DialogFooter className="flex justify-end space-x-2">
                            ...
                            <Button type="submit" onClick={courseForm.handleSubmit(onCourseSubmit)}>{t("next")}</Button>
                        </DialogFooter>
                    </Form>
                )}
                ...
                {stage === 1 && (
                    ...
                    >
                                    ...
                                    return (
                                        <Fragment key={`${currentQuestionIndex}-${idx}`}>
                                            <FormField
                                                control={questionForm.control}
                                                name={`answer${idx}`}
                                                rules={{ required: `${t("answer")} #${idx + 1} is required.` }}
                                                render={({ field }) => (
                                                    <FormItem>
                                                                ...
                                                                onCheckedChange={(chk) => {
                                                                    if (questionType === QuestionType.SINGLE_CHOICE) {
                                                                        ...
                                                                        range(answersAmount).forEach((j) => {
                                                                            questionForm.setValue(`isCorrect${j}`, false);
                                                                        });
                                                                    }
                                                                    field.onChange(chk);
                                                                }}
                                    ...
                                    {answersAmount > 2 && (
                                        <Button
                                            onClick={() => {
                                                ...
                                                questionForm.unregister(`isCorrect${answersAmount - 1}`);
                        ...
                        <DialogFooter className="flex justify-end space-x-2">
                            ...
                            <Button
                                onClick={() => {
                                    const errs = validateQuestion(questionForm.getValues(), Boolean(selectedCourse));
                                    if (errs) {
