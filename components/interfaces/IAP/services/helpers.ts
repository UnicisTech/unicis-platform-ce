...
export const getCourseStatus = (
  ...
  if (teamCourse.progress.length < membersToPassCourse.length)
    return 'inprogress';
  if (
    teamCourse.progress.length === membersToPassCourse.length &&
    teamCourse.progress.every(({ progress }) => progress === 100)
  )
    return 'done';
  return 'todo';
};

export const findMemberProgressInTeamCourse = (
  teamCourse: TeamCourseWithProgress,
  memberId: string
): CourseProgress | undefined =>
  teamCourse.progress.find(({ teamMemberId }) => teamMemberId === memberId);
