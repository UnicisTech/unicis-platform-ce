...
export default async function handler(
  ...
  try {
    ...
    if (!fleetBase || !fleetToken) {
      return res
        .status(500)
        .json({ error: 'Fleet not configured or not authenticated' });
    }
    ...
    if (getRes.status === 403) {
      console.log(
        `[EnsureSecret] User ${userId} doesn't have permission to access secret (member role)`
      );
    ...
    if (getRes.status === 404) {
      ...
      if (createRes.ok) {
        ...
        console.log(
          `[EnsureSecret] Created secret for user ${userId} in team ${teamId}`
        );
        return res.status(201).json({ created: true, secret });
      } else {
        const errorData = await createRes.json().catch(() => ({}));
        console.error(
          '[EnsureSecret] Failed to create secret:',
          createRes.status,
          errorData
        );
        return res
          .status(500)
          .json({ error: 'Failed to create secret', details: errorData });
      }
    }
