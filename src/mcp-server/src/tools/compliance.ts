...
Returns: List of controls with their current status and notes.`,
      inputSchema: z
        .object({
          ...
          framework: z
            .string()
            .describe("Framework ID, e.g. 'iso-2022', 'mvsp', 'nist', 'nis2'"),
        })
        .strict(),
      annotations: {
        readOnlyHint: true,
    ...
    async ({ slug, framework }) => {
      const resp = await apiGet<CscGetResponse>(
        `/api/teams/${slug}/csc/${framework}`
      );
      ...
      if (!entries.length) {
        return {
          content: [
            {
              type: 'text',
              text: `No control statuses found for framework: ${framework}`,
            },
          ],
        };
      }
      const lines = entries.map(
        ([controlId, status]) => `- **${controlId}**: ${status}`
      );
      return {
        content: [
          {
            type: 'text',
            text: `## ${framework} Controls (${entries.length})\n\n${lines.join('\n')}`,
          },
        ],
        structuredContent: {
          framework,
...
Returns: Updated control status.`,
      inputSchema: z
        .object({
          ...
          framework: z
            .string()
            .describe("Framework identifier, e.g. 'iso-2022', 'mvsp'"),
          status: z
            .enum([
              'unknown',
              'not-applicable',
    ...
    async ({ slug, controlId, framework, status }) => {
      ...
      return {
        content: [
          {
            type: 'text',
            text: `✅ Control **${controlId}** updated to **${updatedStatus}**`,
          },
        ],
        structuredContent: {
          controlId,
...
Returns: Array of API key metadata (id, name, createdAt). Token values are never returned.`,
    ...
    async ({ slug }) => {
      ...
      if (!keys.length) {
        return {
          content: [{ type: 'text', text: 'No API keys found for this team.' }],
        };
      }
      const lines = keys.map(
        (k) =>
          `- **${k.name}** (id: ${k.id}) — created ${new Date(k.createdAt).toLocaleDateString()}`
      );
      return {
        content: [
          {
...
Returns: New API key object including the token value (shown once only).`,
      inputSchema: z
        .object({
          ...
          name: z
            ...
            .min(1)
            .max(100)
            .describe('Descriptive name for the key'),
        })
        .strict(),
      annotations: {
        readOnlyHint: false,
    ...
    async ({ slug, name }) => {
      const key = await apiPost<ApiKey & { token: string }>(
        `/api/teams/${slug}/api-keys`,
        { name }
      );
      return {
        content: [
