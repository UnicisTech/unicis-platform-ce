    ...
    async ({ slug, status, priority }) => {
      ...
      return {
        content: [
          {
            type: 'text',
            text: `## Tasks (${filtered.length}/${tasks.length})\n\n${lines.join('\n\n')}`,
          },
        ],
      ...
      inputSchema: z
        .object({
          ...
          controls: z
            .array(z.string())
            .optional()
            .describe('Control IDs to assign immediately'),
        })
        .strict(),
      annotations: {
        readOnlyHint: false,
    ...
    async ({
      ...
      return {
        content: [
          {
            type: 'text',
            text: `✅ Task #${task.taskNumber} created: **${task.title}**\nStatus: ${task.status} | Priority: ${task.priority}`,
          },
        ],
        structuredContent: task,
      };
      ...
      inputSchema: z
        .object({
          ...
          taskNumber: z
            ...
            .int()
            .positive()
            .describe('Task number to update'),
    ...
    async ({ slug, taskNumber, ...updates }) => {
      ...
      return {
        content: [
          {
            type: 'text',
            text: `✅ Task #${task.taskNumber} updated: **${task.title}**\nStatus: ${task.status} | Priority: ${task.priority} | Due: ${formatDate(task.duedate)}`,
          },
        ],
        structuredContent: task,
      };
      ...
      inputSchema: z
        .object({
          ...
          taskNumber: z
            ...
            .int()
            .positive()
            .describe('Task number to delete'),
        })
        .strict(),
      annotations: {
        readOnlyHint: false,
    ...
    async ({ slug, taskNumber }) => {
      ...
      return {
        content: [
          {
            type: 'text',
            text: `🗑️ Task #${taskNumber} has been permanently deleted.`,
          },
        ],
      };
    }
...
Returns: Confirmation.`,
    ...
    async ({ slug, taskNumber, controls, operation, ISO }) => {
      await apiPut(`/api/teams/${slug}/tasks/${taskNumber}/csc`, {
        controls,
        operation,
        ISO,
      });
...
Returns: Created comment object.`,
    ...
    async ({ slug, taskNumber, text }) => {
      const comment = await apiPost<Record<string, unknown>>(
        `/api/teams/${slug}/tasks/${taskNumber}/comments`,
        { text }
      );
      return {
        content: [
          { type: 'text', text: `✅ Comment added to task #${taskNumber}.` },
        ],
...
Returns: Array of attachment objects with id, filename, url.`,
    ...
    async ({ slug, taskNumber }) => {
      ...
      if (!attachments.length) {
        return {
          content: [
            { type: 'text', text: `No attachments on task #${taskNumber}.` },
          ],
        };
      }
      const lines = attachments.map(
        (a) => `- **${a.filename}** (id: \`${a.id}\`)`
      );
      return {
        content: [
          {
            type: 'text',
            text: `## Attachments on task #${taskNumber} (${attachments.length})\n\n${lines.join('\n')}`,
          },
        ],
    ...
    async ({ slug, taskNumber, filename, content, mimeType }) => {
      ...
      return {
        content: [
          {
            type: 'text',
            text: `✅ File "${filename}" uploaded to task #${taskNumber}.\nURL: ${(result as { url?: string })?.url ?? 'N/A'}`,
          },
        ],
...
Returns: Confirmation.`,
      inputSchema: z
        .object({
          ...
          attachmentId: z
            .string()
            .min(1)
            .describe('Attachment ID from unicis_list_attachments'),
        })
        .strict(),
      annotations: {
        readOnlyHint: false,
    ...
    async ({ slug, taskNumber, attachmentId }) => {
      await apiDelete(
        `/api/teams/${slug}/tasks/${taskNumber}/attachments?id=${attachmentId}`
      );
      return {
        content: [
          {
            type: 'text',
            text: `✅ Attachment \`${attachmentId}\` removed from task #${taskNumber}.`,
          },
        ],
      };
    }
