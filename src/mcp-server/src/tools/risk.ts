      ...
      if (!risk?.length) {
        return {
          content: [
            {
              type: 'text',
              text: `No risk assessment linked to task #${taskNumber}.`,
            },
          ],
        };
      }
        ...
        .object({
          ...
          risk: z
            .object({
              ...
              AssetOwner: z
                .string()
                .describe('Asset owner (person responsible)'),
              Impact: z.string().describe('Impact description'),
              RawProbability: z
                .number()
                .min(0)
                .max(100)
                .describe('Raw probability score 0-100'),
              RawImpact: z
                .number()
                .min(0)
                .max(100)
                .describe('Raw impact score 0-100'),
            })
          ...
          treatment: z
            .object({
              RiskTreatment: z
                .string()
                .describe(
                  'Treatment strategy (e.g. mitigate, accept, transfer)'
                ),
              ...
              TreatmentStatus: z
                ...
                .min(0)
                .max(100)
                .describe('Treatment completion percentage 0-100'),
              TreatedProbability: z
                .number()
                .min(0)
                .max(100)
                .describe('Residual probability after treatment 0-100'),
              TreatedImpact: z
                .number()
                .min(0)
                .max(100)
                .describe('Residual impact after treatment 0-100'),
            })
    ...
    async ({ slug, taskNumber }) => {
      ...
      return {
        content: [
          {
            type: 'text',
            text: `✅ Risk assessment removed from task #${taskNumber}.`,
          },
        ],
      };
    }
...
Returns: Tasks with risk data, sorted by descending raw score.`,
    ...
    async ({ slug, minRawScore }) => {
      ...
      if (!withRisk.length) {
        return {
          content: [{ type: 'text', text: 'No risk assessments found.' }],
        };
      }

      const lines = withRisk.map(
        (r) =>
      ...
      return {
        content: [
          {
            type: 'text',
            text: `## Risk Register (${withRisk.length} items)\n\n${lines.join('\n\n')}`,
          },
        ],
        structuredContent: {
          items: withRisk.map((r) => ({
