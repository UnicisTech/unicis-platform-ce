...
export function downloadTaskTemplateOds(): void {
  ...
  ws.B2.c = [{ a: 'Unicis', t: `Valid values:\n${VALID_STATUSES.join('\n')}` }];
  if (!ws.C2) ws.C2 = { v: DEFAULT_TASK_PRIORITY, t: 's' };
  ws.C2.c = [
...
async function parseOdsImport(file: File): Promise<TaskImportRow[]> {
  ...
  return data.slice(1).reduce<TaskImportRow[]>((acc, row) => {
    ...
    const priority =
      priorityIndex >= 0
        ? String(row[priorityIndex] ?? '')
            .trim()
            .toLowerCase()
        : '';
    const rawDate = row[dueDateIndex];
    const duedate = rawDate ? String(rawDate).trim() : '';
    if (!title && !status && !priority && !duedate) return acc;
    acc.push(validateImportRow({ title, status, priority, duedate }));
    return acc;
  }, []);
}
