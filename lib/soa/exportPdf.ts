...
function statusFg(s: string): RGB { return STATUS_FG[s] ?? defaultFg; }
...
export function downloadSoaPdf(payload: SoaPayload): void {
  ...
  doc.setFontSize(13);
  doc.text(`Statement of Applicability — ${meta.framework}`, margin + 8, y + 18);
  y += 48;
  ...
  doc.text(`Organisation: ${meta.teamName}`, margin, y);
  doc.text(
    `Date of Export: ${format(meta.dateOfExport, 'dd MMM yyyy HH:mm')}`,
    pageW - margin,
    y,
    { align: 'right' }
  );
  ...
  autoTable(doc, {
    ...
    head: [['Code', 'Section', 'Control', 'Requirements', 'Status']],
    ...
    didDrawPage(data) {
      ...
      doc.setFont('helvetica', 'normal');
      doc.text(
        `Generated by Unicis Platform  •  ${format(meta.dateOfExport, 'dd MMM yyyy HH:mm')}`,
        pageW / 2,
