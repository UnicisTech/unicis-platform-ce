...
export function exportRpaCsv(
  ...
  const rows = tasks.map((task) => {
    ...
    return [
      ...
      d.reviewDate,
      d.dataTransfer,
      csvEsc(d.specialCategories),
    ].join(',');
  });
  const csv = [headers.join(','), ...rows].join('\r\n');
  triggerDownload(
    new Blob([csv], { type: 'text/csv;charset=utf-8' }),
    `RPA_${teamName}_${datestamp()}.csv`
  );
}

// ── HTML ─────────────────────────────────────────────────────────────────────

export function exportRpaHtml(
  tasks: TaskWithRpaProcedure[],
  teamName: string,
  t: TFunc
) {
...
.meta{background:var(--meta);border:1px solid var(--bdr);border-top:none;padding:8px 20px;display:flex;justify-content:space-between;font-size:11px;margin-bottom:16px;border-radius:0 0 6px 6px;flex-wrap:wrap;gap:8px}
.table-wrapper{overflow-x:auto;margin-top:4px;border-radius:0 0 6px 6px;-webkit-overflow-scrolling:touch}
table{width:100%;border-collapse:collapse;font-size:11px;min-width:100%}
th{background:var(--mid);color:#fff;padding:9px 10px;text-align:left;position:sticky;top:0;z-index:1;word-wrap:break-word;white-space:normal}
td{padding:8px 10px;border-bottom:1px solid var(--bdr);vertical-align:top;word-wrap:break-word}
tr.alt td{background:var(--alt)}
td.mono{font-family:monospace}
.badge{display:inline-block;padding:3px 8px;border-radius:12px;font-size:10px;font-weight:bold;word-wrap:break-word}
.footer{margin-top:20px;font-size:10px;color:#888;text-align:center}
@media(max-width:768px){body{padding:12px}.hdr h1{font-size:14px}.meta{font-size:10px;padding:6px 10px}.meta>div{flex:1 1 100%}.table-wrapper{border-radius:0;margin:0 -12px}.table-wrapper table{font-size:9px;min-width:100%}th,td{padding:5px 6px;font-size:8px}.badge{font-size:7px;padding:2px 4px}}
@media(max-width:480px){body{padding:8px}.hdr h1{font-size:12px}.meta{font-size:9px;padding:4px 8px}table{font-size:8px}th,td{padding:4px 5px;font-size:7px}.badge{font-size:6px;padding:1px 3px}}
...
<div class="table-wrapper"><table><thead><tr>${headers.map((h) => `<th>${escHtml(h)}</th>`).join('')}</tr></thead><tbody>${tableRows}</tbody></table></div>
<div class="footer">Generated by Unicis Platform &nbsp;•&nbsp; ${timestamp()} &nbsp;•&nbsp; ${tasks.length} record(s)</div>
</body></html>`;

  triggerDownload(
    new Blob([html], { type: 'text/html;charset=utf-8' }),
    `RPA_${teamName}_${datestamp()}.html`
  );
}

// ── PDF ──────────────────────────────────────────────────────────────────────

export function exportRpaPdf(
  tasks: TaskWithRpaProcedure[],
  teamName: string,
  t: TFunc
) {
