...
function tiaRow(task: TaskWithTiaProcedure, t: TFunc) {
  ...
  return {
    ...
    endDate: getEndDate(
      p[0]?.StartDateAssessment ?? '',
      p[0]?.AssessmentYears ?? 1
    ),
...
export function exportTiaCsv(
  ...
  const rows = tasks.map((task) => {
    ...
    return [
      d.id,
      csvEsc(d.title),
      csvEsc(d.dataExporter),
      csvEsc(d.dataImporter),
      d.assessmentDate,
      d.endDate,
      csvEsc(d.transferPermitted),
    ].join(',');
  });
  const csv = [headers.join(','), ...rows].join('\r\n');
  triggerDownload(
    new Blob([csv], { type: 'text/csv;charset=utf-8' }),
    `TIA_${teamName}_${datestamp()}.csv`
  );
}

// ── HTML ─────────────────────────────────────────────────────────────────────

export function exportTiaHtml(
  tasks: TaskWithTiaProcedure[],
  teamName: string,
  t: TFunc
) {
...
<div class="footer">Generated by Unicis Platform &nbsp;•&nbsp; ${timestamp()} &nbsp;•&nbsp; ${tasks.length} record(s)</div>
  ...
  triggerDownload(
    ...
    `TIA_${teamName}_${datestamp()}.html`
  );
}

// ── PDF ──────────────────────────────────────────────────────────────────────

export function exportTiaPdf(
  tasks: TaskWithTiaProcedure[],
  teamName: string,
  t: TFunc
) {
